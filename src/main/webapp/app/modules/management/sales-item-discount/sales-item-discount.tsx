...
export const SalesItemDiscount = () => {
  ...
  const salesItemDiscountsForPage = useAppSelector(state => state.salesItemDiscounts.salesItemDiscountsForPage);
  const totalItems = useAppSelector(state => state.salesItemDiscounts.totalItems);
  ...
  useEffect(() => {
    if (shops.length > 0) {
      ...
      // dispatch(getShopSalesItemDiscounts({ shopId: shops[0].id }));
    }
  }, [shops]);
  ...
  return (
    <div>
      <h2 id="sales-item-page-heading" data-cy="salesItemDiscountPageHeading">
        ...
        <div className="d-flex justify-content-end">
          ...
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            ...
            <Translate contentKey="salesItemDiscount.home.refreshListLabel">할인 조회</Translate>
          </Button>
          <Link to={`${selectedValue}/new`} className="btn btn-primary jh-create-entity">
            <FontAwesomeIcon icon="plus" /> <Translate contentKey="salesItemDiscount.home.createLabel">할인 생성</Translate>
          </Link>
        </div>
      </h2>
      <Table responsive striped>
        <thead>
          <tr>
            ...
            <th className="hand" onClick={sort('shopCommissionPrice')}>
              <Translate contentKey="global.label.shopCommissionPrice">금액</Translate>
              <FontAwesomeIcon icon={getSortIconByFieldName('shopCommissionPrice')} />
            </th>
            <th className="hand" onClick={sort('modelCommissionPrice')}>
              <Translate contentKey="global.label.modelCommissionPrice">금액</Translate>
              <FontAwesomeIcon icon={getSortIconByFieldName('modelCommissionPrice')} />
            </th>
            <th className="hand" onClick={sort('mamaCommissionPrice')}>
              <Translate contentKey="global.label.mamaCommissionPrice">금액</Translate>
              <FontAwesomeIcon icon={getSortIconByFieldName('mamaCommissionPrice')} />
            </th>
            <th>
        ...
        <tbody>
          ...
          {salesItemDiscountsForPage.map((user, i) => (
            <tr id={user.id} key={`user-${i}`}>
              ...
              <td>{user.nameEn}</td>
              <td>{user.price}</td>
              <td>{user.shopCommissionPrice}</td>
              <td>{user.modelCommissionPrice}</td>
              <td>{user.mamaCommissionPrice}</td>
              <td>
                {user.activated ? (
      ...
      {totalItems ? (
        <div className={salesItemDiscountsForPage?.length > 0 ? '' : 'd-none'}>
...
export default SalesItemDiscount;
